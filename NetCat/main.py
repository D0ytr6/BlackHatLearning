...
def execute(cmd):
    cmd = cmd.strip()  # clear str spaces by strip
    print(cmd)
    if not cmd:
        return
    output = subprocess.check_output(shlex.split(cmd),
                                     stderr=subprocess.STDOUT, shell=True)  # split string in Posix style and run command, return error output
        ...
        try:
            while True:
                ...
                if response:
                    print(response)
                    buffer = input('> ')
                    buffer += '\n' # auto add in original netcat program
...
if __name__ == '__main__':
    # print(execute("ping 192.168.123.1"))
 ...
 '''))
    ...
    if args.listen:
        ...
        buffer = sys.stdin.read() # Win EOF Create by Ctrl+Z
        print(buffer)
